...
class IndexData : public GloveObject {
	GLOVE_MEM_ALLOC_FUNCS("IndexData")
public:
	IndexData();
	virtual ~IndexData();

	/** Returns the number of indices */
	virtual const size_t GetIndexCount() const { return indexCount; }
	/** Sets the number of indices */
	virtual void SetIndexCount(size_t count) { indexCount = count; }

	/** Returns the hardware/GPU buffer holding the index data */
	virtual const GPUBufferPtr GetBuffer() const { return indexBuffer; }
