...
template <class VertexLayoutType> class ManagedVertexData : public VertexData {
  ...
  public:
    ...
    ManagedVertexData(const GpuBufferManagerPtr& gpuBufferManager, const VertexLayout& vertexLayout)
            : gpuBufferManager(gpuBufferManager), VertexData(), EnableProfilable() {
    ...
    virtual void FlushBuffer() { GetBuffer(0)->WriteData(sizeof(VertexLayoutType) * GetVertexCount(), &vertices[0]); }
