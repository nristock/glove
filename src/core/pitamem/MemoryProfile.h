...
#include "core/pitamem/MemoryGroup.h"

#define EnableProfilable() __memProfilerInterface(this, MG_GENERIC)
#define Profilable()                                                                                                   \
  private:                                                                                                             \
    ::glove::memory::ProfilableBase __memProfilerInterface;
...
*  If more than one MemoryProfile is constructed for the same object a linked list of corresponding MemoryProfiles is
* created.
*/
class MemoryProfile : public std::enable_shared_from_this<MemoryProfile> {
  public:
    ...
    struct SiblingIterator {
        ...
        bool HasNext() { return !(sibling->sibling == nullptr); }

        /** Returns true if the current sibling is null */
        bool IsEnd() { return sibling == nullptr; }
        ...
        MemoryProfile* operator*() { return sibling; }
    };

    /**
     * Creates a self-registereing memory profile using the provided type information, size, root and memory group.
     * See MemoryProfile(const std::type_info& typeinf, unsigned long sizeInBytes, void* root, MemoryGroup memGroup,
     * bool autoregister) for more information.
    */
     ...
     * @param [in] root The object's location in memory. This parameter is used to determine whether a MemoryProfile for
     * this object has already been created.
     * @param [in] memGroup The object's memory group. This parameter is used to provide grouped diagnostics output from
     * the profiler.
    ...
    void* GetMemoryRoot() const { return rootMemory; }

    /** Returns the assigned memory group. */
    MemoryGroup GetMemoryGroup() const { return memGroup; }
    ...
    MemoryProfile* GetImmediateSibling() const { return sibling; }
