...
int main(int argc, const char** argv) {
	...
	IShaderPtr vertexShader = renderSubsystem->GetShaderFactory(renderer)->CreateVertexShader(gEnv->MakeDataPath("data/DefaultShader.vertex"));
	IShaderPtr fragmentShader = renderSubsystem->GetShaderFactory(renderer)->CreateFragmentShader(gEnv->MakeDataPath("data/DefaultShader.fragment"));
	...
	IVertexDataPtr vertexData(new DynamicVertexData<VertexLayouts::PositionColor>(renderSubsystem->GetGpuBufferFactory(renderer), VertexLayouts::PositionColor::GetLayout()));


	std::shared_ptr<DynamicIndexData<unsigned int>> dynIndexData = std::dynamic_pointer_cast<DynamicIndexData<unsigned int>>(indexData);
	std::shared_ptr<DynamicVertexData<VertexLayouts::PositionColor>> dynVertexData = std::dynamic_pointer_cast<DynamicVertexData<VertexLayouts::PositionColor>>(vertexData);
