...
GLRenderTarget::GLRenderTarget(const Dimensions& targetDimensions) : targetDimensions(targetDimensions) {
    GL::GenFramebuffers(1, &framebufferId);

    GL::GenTextures(1, &targetTexture);
    GL::BindTexture(GL_TEXTURE_2D, targetTexture);

    GL::TexImage2D(GL_TEXTURE_2D, 0, GL_RGB, static_cast<GLsizei>(targetDimensions.GetWidth()), static_cast<GLsizei>(targetDimensions.GetHeight()), 0, GL_RGB, GL_UNSIGNED_BYTE, 0);
...
const Dimensions& GLRenderTarget::GetDimensions() const {
